...
function change() {
	...
	if(type == 'dog') {
		location.href="/member/pet?type=1";
	} else {
		location.href="/member/pet?type=2";
	}
}
							...
							<tr>
								<td align=center>
									...
									<table style="width:500px;border:0px;">
										<tr>
											<td align=center>
												<input type="radio" name="pet" id="pet_dog" value="dog" onchange="change()" <c:if test="${type != '2'}">checked</c:if>> <label for="pet_dog"><a href="/member/pet?type=1">강아지</a></label>
												&nbsp;&nbsp;&nbsp;
												<input type="radio" name="pet" id="pet_cat" value="cat" onchange="change()" <c:if test="${type == '2'}">checked</c:if>> <label for="pet_cat"><a href="/member/pet?type=2">고양이</a></label>
											</td>
										</tr>
										<tr>
										...
										<c:choose>
											<c:when test="${type == '2'}">
												<%@ include file="/include/petcat.jsp" %>
											</c:when>
											<c:otherwise>
												<%@ include file="/include/petdog.jsp" %>
											</c:otherwise>
										</c:choose>
										<tr>
											<td><div style="height:40px;"></div></td>
										...
										<tr>
											<td align=center>
												<input class="btn_form" type="submit" value="저장">
											</td>
										</tr>
									</table>
									</form>
